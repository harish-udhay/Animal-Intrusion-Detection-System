...
void distributeSensors()
{
	...
	for(i=1; i<numberX - 1 ; i++)
		for(j=1; j<numberY - 1 ; j++)
		{
			...
			CameraSensorArray[CameraSensorNumber].sensorPan = 0;
			CameraSensorArray[CameraSensorNumber].sensorTilt = 0;
...
void sendCameraSensor(CameraSensor *c,int i)
{
		char x[100];
		char temp[10];
		setMessagePacket(&G,"","","","");
...
void computeRange(float ScaleFactor)
{
	...
	for(i=0;i<CameraSensorNumber;i++)
	{
...

		{
			...
			CameraSensorArray[i].sensorTilt = 0;
		...
		else
		{
			...
			CameraSensorArray[i].sensorPan = 0;
			CameraSensorArray[i].sensorTilt = 0;
		}
	}
}
...
void intrusionDetected(int X,int Y,char C[50])
{
	...
	for(i=0;i<CameraSensorNumber;i++)				
		sendCameraSensor(&CameraSensorArray[i],i);
